...
class FamaFrench:
    ...
    def OOSRollingWindowPermnos(self, save=True, printOnConsole=True, initialOOSYear=1998, sizeWindow=60, cap=0.01,
        ...
        for factor in listFactors:
            ...
            for t in range(T - firstOOSDailyIdx):
                ...
                idxsNotMissingValues = ~np.any(
                    np.isnan(Rdaily[(t + firstOOSDailyIdx - sizeWindow):(t + firstOOSDailyIdx), :]), axis=0).ravel()
                ...
                if factor == 0:
                    ...
                    factors = FamaFrenchDaily[(t + firstOOSFFDailyIdx):(t + firstOOSFFDailyIdx + 1),
                              :factor]  # TxnFactors
                    ...
                    if t % 50 == 0 and printOnConsole:
                        concatenate = np.concatenate(
                            (np.nan_to_num(Rdaily[(t + firstOOSDailyIdx), assetsToConsider], copy=False),
