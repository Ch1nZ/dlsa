...
class PCA:
    ...
    def OOSRollingWindowPermnos(self, save=True, printOnConsole=True, initialOOSYear=1998, sizeWindow=60,
                                sizeCovarianceWindow=252, CapProportion=0.01, factorList=range(0, 16)):
        ...
        for factor in factorList:
            ...
            for t in range(T - firstOOSDailyIdx):
                ...
                if factor == 0:
                    ...
                    loadings = eigenVectors[:,
                               idxs].real  # takes eigenvector corresponding to factor largest eigenvalues
                    ...
                    regr = LinearRegression(fit_intercept=False, n_jobs=48).fit(factors,
                                                                                res_cov_window[-sizeWindow:, :])
                    loadings = regr.coef_
                ...
                if t % 50 == 0 and printOnConsole:
                    ...
                    print(np.linalg.norm(
                        residualsOOS[t:(t + 1), assetsToConsider] - residualsMatricesOOS[t:(t + 1)] @ Rdaily[
                            (t + firstOOSDailyIdx), assetsToConsider]))
            ...
            if save:
                if factor == 20:
                    np.save(os.path.join(self._logdir,
    ...
    def OOSRollingWindowPermnosVectorized(self, save=True, printOnConsole=True, initialOOSYear=1998, sizeWindow=60,
                                          sizeCovarianceWindow=252, CapProportion=0.01, factorList=range(0, 16)):
        ...
        for t in range(T - firstOOSDailyIdx):
            ...
            idxsNotMissingValues = ~np.any(
                np.isnan(Rdaily[(t + firstOOSDailyIdx - sizeCovarianceWindow + 1):(t + firstOOSDailyIdx + 1), :]),
                axis=0).ravel()
            ...
            res_cov_window = Rdaily[(t + firstOOSDailyIdx - sizeCovarianceWindow + 1):(t + firstOOSDailyIdx + 1),
                             idxsSelected]
            res_mean = np.mean(res_cov_window, axis=0, keepdims=True)
            res_vol = np.sqrt(np.mean((res_cov_window - res_mean) ** 2, axis=0, keepdims=True))
            res_normalized = (res_cov_window - res_mean) / res_vol
            ...
            for (i, factor) in enumerate(factorList):
                if factor == 0:
                    residualsOOS[i:(i + 1), t:(t + 1), idxsSelected] = Rdaily[(t + firstOOSDailyIdx):(
                                t + firstOOSDailyIdx + 1), idxsSelected]
                    ...
                    factors = np.dot(res_cov_window[-sizeWindow:, :] / res_vol, loadings)
                    old_loadings = loadings
                    regr = LinearRegression(fit_intercept=False, n_jobs=-1).fit(factors,
                                                                                res_cov_window[-sizeWindow:, :])
                    loadings = regr.coef_
